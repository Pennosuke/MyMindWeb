...
export default class spwbResultScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Usersdata: [],
  ...
  async componentDidMount() {
    const datasnapshot = await db.collection('แบบวัดสุขภาวะทางจิตใจ').where('userName', '==', global.userData.userName).get();
    if (datasnapshot.docs.length > 0) {
      const getUserData = datasnapshot.docs.map(doc => ({
        userName: doc.data().userName,
        timestamp: doc.data().timestamp,
        autonomy: doc.data()['1'].value + doc.data()['7'].value + doc.data()['13'].value,
        enviMaster: doc.data()['2'].value + doc.data()['8'].value + doc.data()['14'].value,
        perGrowth: doc.data()['3'].value + doc.data()['9'].value + doc.data()['15'].value,
        posiRelaWithOthers: doc.data()['4'].value + doc.data()['10'].value + doc.data()['16'].value,
        purInLife: doc.data()['5'].value + doc.data()['11'].value + doc.data()['17'].value,
        selfAccept: doc.data()['6'].value + doc.data()['12'].value + doc.data()['18'].value
      }));
                    ...
                    <View style={styles.surveyContainer}>
                      <View style={{ marginLeft: 10, marginRight: 10 }}>
                        ...
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>ความเป็นตัวของตัวเอง</Text>
                          <Text style={styles.infoText}>{userdata.autonomy}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>การเรียนรู้ด้านสิ่งแวดล้อม</Text>
                          <Text style={styles.infoText}>{userdata.enviMaster}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>การเติบโตส่วนบุคคล</Text>
                          <Text style={styles.infoText}>{userdata.perGrowth}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>การมีสัมพันธภาพทางบวกกับผู้อื่น</Text>
                          <Text style={styles.infoText}>{userdata.posiRelaWithOthers}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>จุดมุ่งหมายชีวิต</Text>
                          <Text style={styles.infoText}>{userdata.purInLife}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>การยอมรับตนเอง</Text>
                          <Text style={styles.infoText}>{userdata.selfAccept}</Text>
                        </View>
                      </View>
                    </View>
                  </View>
