...
export default class dassResultScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Usersdata: [],
  ...
  async componentDidMount() {
    const datasnapshot = await db.collection('แบบสอบถามวัดภาวะสุขภาพจิต').where('userName', '==', global.userData.userName).get();
    if (datasnapshot.docs.length > 0) {
      const getUserData = datasnapshot.docs.map(doc => ({
        userName: doc.data().userName,
        timestamp: doc.data().timestamp,
        depression: doc.data()['3'].value + doc.data()['5'].value + doc.data()['10'].value + doc.data()['13'].value + doc.data()['16'].value + doc.data()['17'].value + doc.data()['21'].value,
        anxiety: doc.data()['2'].value + doc.data()['4'].value + doc.data()['7'].value + doc.data()['9'].value + doc.data()['15'].value + doc.data()['19'].value + doc.data()['20'].value,
        stress: doc.data()['1'].value + doc.data()['6'].value + doc.data()['8'].value + doc.data()['11'].value + doc.data()['12'].value + doc.data()['14'].value + doc.data()['18'].value
      }));
                    ...
                    <View style={styles.surveyContainer}>
                      <View style={{ marginLeft: 10, marginRight: 10 }}>
                        ...
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>ภาวะซึมเศร้า</Text>
                          <Text style={styles.infoText}>{userdata.depression}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>ความวิตกกังวล</Text>
                          <Text style={styles.infoText}>{userdata.anxiety}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>ความเครียด</Text>
                          <Text style={styles.infoText}>{userdata.stress}</Text>
                        </View>
                        <View style={{flexDirection: 'row',justifyContent: 'space-between'}}>
                          <Text style={styles.infoText}>ภาวะสุขภาพจิต</Text>
                          <Text style={styles.infoText}>{this.renderResult(userdata.depression)}</Text>
                        </View>
                      </View>
                    </View>
                  </View>
