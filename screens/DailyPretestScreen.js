...
export default class TreatmentScreen extends Component {
  ...
  _onPlaybackStatusUpdate(playbackStatus, currentHandlerIndex, currentAnswerIndex){
    const state = this.state;
    // console.log('playbackStatus', playbackStatus)
    if(playbackStatus.isPlaying) {
      state.videoHandlers[currentHandlerIndex].value.playTime = (playbackStatus.positionMillis - state.videoHandlers[currentHandlerIndex].value.checkpointVideo)
      this.setState(state);
    }
    else {
      if(state.videoHandlers[currentHandlerIndex].value.totalPlayTime === NaN) {
        state.videoHandlers[currentHandlerIndex].value.totalPlayTime = 0;
      }
      state.videoHandlers[currentHandlerIndex].value.totalPlayTime += state.videoHandlers[currentHandlerIndex].value.playTime;
      state.videoHandlers[currentHandlerIndex].value.checkpointVideo = playbackStatus.positionMillis;
      state.videoHandlers[currentHandlerIndex].value.playTime = 0;

      state.answers[currentAnswerIndex].value.totalSecond = Math.round((state.videoHandlers[currentHandlerIndex].value.totalPlayTime / 1000) * 100) / 100;
      this.setState(state);
    }
    // console.log('state',state);
  };
  ...
  renderVideo(survey,stepIndex) {
    ...
    const currentHandlerIndex = state.videoHandlers.findIndex(ans => ans.contentId === currentContentId);
    if (!state.answers.find(ans => ans.contentId === currentContentId)) {
      // console.log('generate new obj')
      state.answers.push({
        contentId : currentContentId,
        value: {
          totalSecond: 0
    ...
    return (
      <View style={styles.surveyContainer}>
        <View style={{ marginLeft: 10, marginRight: 10 }}>
          <Text style={styles.infoText}>{contentText}</Text>
          <Text style={[styles.infoText,{textAlign: 'center', color: '#444444'}]}>
            { state.videoHandlers[currentHandlerIndex].ready ? 'วิดีโอโหลดเสร็จแล้ว\nถ้ายังไม่เห็นปุ่มเล่นวิดีโอ ลองแตะในกรอบสี่เหลี่ยมนะคะ' : 'กำลังโหลดวิดีโอ...' }
          </Text>
          <Video
            source={videoUri}
            rate={1.0}
            ...
            isLooping={false}
            useNativeControls
            style={{ width: '100%', height: (((windowWidth * 0.5) - 40) * 45 / 80), alignSelf: "center", borderWidth: 1, borderColor: '#444444'}}
            onPlaybackStatusUpdate={(playbackStatus) => this._onPlaybackStatusUpdate(playbackStatus, currentHandlerIndex, currentAnswerIndex)}
            onFullscreenUpdate={this.onFullscreenUpdate}
            onReadyForDisplay={() => {
              state.videoHandlers[currentHandlerIndex].ready = true;
              this.setState(state);
            }}
          />
        </View>
        <View style={styles.navButtonContainerStyle}>
          ...
          }
          {
            this.renderNextOrFinishButton(
              ...
              !!(this.state.videoHandlers[currentHandlerIndex].value.totalPlayTime + this.state.videoHandlers[currentHandlerIndex].value.playTime >= minTime * 1000)
            )
          }
        </View>
      </View>
        ...
        <View style={{ marginLeft: 10, marginRight: 10 }}>
          <Text style={styles.infoText}>{contentText}</Text>
          {
            this.state.answers[currentAnswerIndex].value.map(( question, index ) =>
              <View key={index}>
                <Text style = {[styles.infoText,{ textAlign: 'center', marginLeft: 0, marginBottom: 5 }]}>
              ...
              this.state.answers[currentAnswerIndex].value.map(( ansEmotion, index ) =>
                <View style={{flex: 1, flexDirection: 'row', paddingTop: 20}} key={index}>
                  ...
                  <View style={{flex: 2, alignItems: 'center'}}>
                    <Text style={[styles.infoText,{textAlign: 'center', marginBottom: 10}]}>เลือกระดับความรู้สึก(1-10)</Text>
                    <Picker
  ...
  renderTextInput(survey,stepIndex) {
    ...
    // console.log('this.state', this.state);
    const currentAnswerIndex = state.answers.findIndex(ans => ans.contentId === currentContentId);
    const imageUri = survey[stepIndex].imageUri === undefined ? undefined : survey[stepIndex].imageUri
    return (
      <View style={styles.surveyContainer}>
        <View style={{ marginLeft: 10, marginRight: 10 }}>
          <Text style={styles.infoText}>{contentText}</Text>
              ...
              >
                ...
                >
                  <Text
                    style={
                      this.isDisabledAfterGame(choice.choiceText, state.answers[currentAnswerIndex].value.choices) ?
                      [styles.buttonText,{color: '#a3a3a3'}]
                      : styles.buttonText
                    }
                  >
                    {choice.choiceText}
