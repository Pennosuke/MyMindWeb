...
export class TreatmentSurvey extends Component {
  static propTypes = {
  ...
  renderTextInputGroupElement() {
    ...
    const currentQuestionIndex = this.state.currentQuestionIndex;
    const answers = this.state.answers;
    const { questionText, questionId, subQuestions, defaultValue } = survey[currentQuestionIndex]
  ...
  */
  ...
  renderInfo() {
    ...
    const { questionText, hasImage, imageUri } = survey[currentQuestionIndex];

    return (<View style={containerStyle}>
      {renderInfo(questionText, hasImage, imageUri)}
      {this.renderNavButtons()}
    </View>
    );
  }
